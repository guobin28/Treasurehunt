...
def webhook():
    try:
        ...
        if user not in players:
            players[user] = 0
            reply.body(game[0]["clue"])
            return str(resp)

        level = players[user]

        if level >= len(game):
            reply.body("🎉 You already completed the game!")
            return str(resp)

        level_data = game[level]

        raw_answers = level_data.get("answers") or level_data.get("answer")

        if isinstance(raw_answers, str):
            raw_answers = [raw_answers]

        correct_answers = [normalize(a) for a in raw_answers]

        if msg in correct_answers:
            players[user] += 1
            new_level = players[user]

            success_text = level_data.get("success", "Correct!")
            ...
            if new_level < len(game):
                ...
                reply.body("🏆 Congratulations! This marks the end of the adventure!")
        else:
            reply.body("❌ Wrong answer. Try again!")

        return str(resp)

    except Exception as e:
        print("ERROR:", e)
        return str(MessagingResponse().message("⚠️ Server error"))
